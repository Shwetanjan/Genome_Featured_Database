...
my ($popGenFile, $outFile, @refGeneFile, @popGenFile, $line, $i,$refgeneOutFile,$popSnpOutfile, $tempData1,@tempData2, @tempArray, @rsIdSmple,@rsId1000g);
#$refSeqFile = $ARGV[0];
#$1000gFile = $ARGV[1];
...
#print the lines with valid rsID and with population frequency less than 5% from the popGen file
 foreach $line(@popGenFile) {
  ...
  my $j=0;
  chomp($refGeneFile[0]);
  print OUTER "$refGeneFile[0]  pop_freq\n";
  foreach $var1(@refGeneFile){
      foreach $var2(@rsId1000g){
          if (index($var1, $var2) != -1) {
              #print $var1, "contains", $var2,"\n";
               print $hash{$var2};
               chomp($var1);
               print OUTER "$var1  $hash{$var2}\n";
          }
          $j++;
       }
       $i++;
